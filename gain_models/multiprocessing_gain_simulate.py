...
def simulate_gain_multiprocesses(resolution, unit_cell_length, n_unitcells, frequency_range, PUMP_FREQUENCY, init_amplitudes, I_star,
    ...
    ################################## GAIN PARAMS #######################################
    PUMP_FREQUENCY = utills.functions.toGHz(PUMP_FREQUENCY)
    ...
    def solve(args):
        ...
        return utills.functions.toDb((abs(sol.y[signal][-1]) ** 2) / (abs(sol.y[signal][0]) ** 2))



    # simulate batas and unfold betas*D, then divid by unitcell len to get just beta
    betas_unfolded = utills.functions.beta_unfold(beta_d) / unit_cell_length
