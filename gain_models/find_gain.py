...
from gain_models.microstrip.solve_ode import Solve_ode


def CalculateBetas(FloquetLine, FreqRange):
    Betas = []
    for F in FreqRange:
        a, t, beta, r, x, R, L, G, C = FloquetLine.simulate(F)
        Betas.append(beta)
    return Betas
...
def Calc_Gain(floquet_line, resolution, pump_freq, init_amplitudes, L):
    ...
    # step 1) calc unfolded beta_plt for given range and resolution (betas for signal freq)
