...
class SC_MicroStrip(AbstractSCTL):
    ...
    def __init__(self, width, length, thickness, height, epsilon_r, tan_delta, crit_current):
        ...
        # geometric factors
        self.__g1 = self.__G1(self.__width, self.__sc_height, self.__sc_thickness)
        self.__g2 = self.__G2(self.__width, self.__sc_height, self.__sc_thickness, self.__g1)
    ...
    def __G1(self, sc_width, sc_height, sc_thickness):
        ...
        ra = cmath.exp(
            -1 - cmath.pi * sc_width / (2 * sc_height) - ((p + 1) / cmath.sqrt(p)) * cmath.atanh(1 / cmath.sqrt(p))) * 4 * p / (
                     p - 1)
        ...
        if w_h_ratio >= 5:
            ...
            rb = rb0 - cmath.sqrt((rb0 - 1) * (rb0 - p)) + (p + 1) * cmath.atanh(cmath.sqrt((rb0 - p) / (rb0 - 1))) \
                 - 2 * cmath.sqrt(p) * cmath.atanh(cmath.sqrt((rb0 - p) / (p * (rb0 - 1)))) + cmath.pi * cmath.sqrt(p) * sc_width / (
                         2 * sc_height)
    ...
    def get_gamma_Zc(self, frequency, surface_impedance):
        ...
        Y = self.shunt_admittance_Y(self.__g1, frequency, self.__epsilon_r, self.__tan_delta, self.__sc_height, self.__width)
