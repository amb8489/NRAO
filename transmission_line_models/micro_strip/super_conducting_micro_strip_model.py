...
from utills.constants import K0, Z0, PI, PI2, PI4, C, MU_0, PLANCK_CONST_REDUCEDev
from utills.functions import coth, sech
...
class SuperConductingMicroStripModel(AbstractSCTL):
    ...
    def clac_g1_g2(self,w, h, t):
        ...
        ra = np.exp(-1 - np.pi * w / (2 * h) - ((p + 1) / np.sqrt(p)) * np.arctanh(1 / np.sqrt(p))) * 4 * p / (p - 1)

        Lambda = np.sqrt(p) * (
                    np.pi * w / (2 * h) + (1 + np.log(4 / (p - 1))) * (p + 1) / (2 * np.sqrt(p)) - 2 * np.arctanh(
                1 / np.sqrt(p)))
        Gamma = max(Lambda, p)
        ...
        if w_h_ratio >= 5:
            ...
            rb = rb0 - np.sqrt((rb0 - 1) * (rb0 - p)) + (p + 1) * np.arctanh(np.sqrt((rb0 - p) / (rb0 - 1))) \
                 - 2 * np.sqrt(p) * np.arctanh(np.sqrt((rb0 - p) / (p * (rb0 - 1)))) + np.pi * np.sqrt(p) * w / (2 * h)

        g1 = (np.pi / 2) / (np.log(2 * rb / ra))  # [g1] = 1

        ## geometric factor 2:
        I_l = np.log((2 * p + 2 * np.sqrt(p * (1 - ra) * (p - ra)) - (p + 1) * ra) / (ra * (p - 1)))
        if w_h_ratio < 2:
            K_MS = 2 * h * np.log(rb / ra)
        else:
            K_MS = 2 * h * np.log(2 * rb / ra)

        psi_l = (I_l + np.pi / 2) / K_MS

        g2_l = psi_l * g1  # [g2] = [psi] = 1/length
