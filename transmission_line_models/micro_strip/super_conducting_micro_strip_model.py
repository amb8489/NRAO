...
from utills.constants import PI, K0, Z0, PI2, PI4, PLANCK_CONST_REDUCEDev, MU_0, C
...
class SuperConductingMicroStripModel(AbstractSCTL):
    ...
    def G1(self, sc_width, sc_height, sc_thickness):
        ...
        return (PI / 2) * (1 / (2 * cmath.log(2 * rb / ra)))
    ...
    def G2(self, sc_width, sc_height, sc_thickness, g1):
        ...
        PIW_div_2H = (PI * sc_width) / (2 * sc_height)
        P_plus_1_div_2sqrt_p =(p + 1) / (2 * cmath.sqrt(p))

        Lambda = cmath.sqrt(p) * (PIW_div_2H + P_plus_1_div_2sqrt_p * (
                1 + cmath.log(4 / (p - 1))) - 2 * np.arctanh(1 / cmath.sqrt(p)))
        ...
        K_ms = (2 * sc_height * cmath.log(rb / ra)) if sc_width / sc_height < 2 else (
                2 * sc_height * cmath.log(2 * rb / ra))

        I = cmath.log(((2 * p) + 2 * cmath.sqrt(p * (1 - ra) * (p - ra)) - (p + 1) * ra) / (ra * (p - 1)))

        return (g1 / K_ms) * (I + (PI / 2))
